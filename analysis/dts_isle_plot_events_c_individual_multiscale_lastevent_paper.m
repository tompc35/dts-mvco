...
for kk = 23:23
    ...
    jj = ti(kk);
    t1 = event_daten(jj)-0.33;
    t2 = event_daten(jj)+0.35;
    ...
    if ~isnan(tempC(zic,di))
        ...
        xlabel('\Deltad in eastward direction [m]')
        ylabel(['[m/s]'])
...
figure
subplot(211)
pcolor(mitime_events,C.M.z+0.75,real(squeeze(mean(wc_events,3)))'), shading flat
caxiscen;
cax = caxis;
hold on
contour(datetime_events, 15-zt_all, squeeze(mean([tc_events tsgc_events],3))',[0:.1:100],'color',[0.5 0.5 0.5])
contour(datetime_events, 15-zt_all, squeeze(mean([tc_events tsgc_events],3))',[0:.5:100],'color',[0.3 0.3 0.3])
ylim([0 15])
colorbar

subplot(212)
pcolor(mitime_events,C.M.z+0.75,imag(squeeze(mean(wc_events,3)))'), shading flat
caxiscen;
cax = caxis;
hold on
contour(datetime_events, 15-zt_all, squeeze(mean([tc_events tsgc_events],3))',[0:.1:100],'color',[0.5 0.5 0.5])
...
figure
subplot(211)
plot(mitime_events,real(squeeze(wc_events(:,1,:)))'), shading flat

subplot(212)
plot(mitime_events,imag(squeeze(wc_events(:,1,:)))'), shading flat
